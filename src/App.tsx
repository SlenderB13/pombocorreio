...
function App() {
  ...
  async function pickFiles() {
    try {
      ...
      setStatus(`Could not open file picker: ${error}`);
    }
  }
    ...
    <main>
      ...
      {snapshot.incoming.map((offer) => (
        <section className="incoming" key={offer.id}>
          ...
          <div className="actions">
            ...
            <button onClick={() => answer(offer, true)}>
              <Check size={16} /> Accept & trust
            </button>
          </div>
        </section>
      ))}
      ...
      <section className="devices">
        ...
        {snapshot.peers.length === 0 ? (
          <div className="empty">
            Open Pombo Correio on another device connected to this network.
          </div>
        ) : (
          snapshot.peers.map((peer: Peer) => (
            <label
              className={`device ${selected.includes(peer.id) ? "selected" : ""}`}
              key={peer.id}
            >
              <input
                type="checkbox"
                ...
                onChange={() =>
                  setSelected((items) =>
                    items.includes(peer.id)
                      ? items.filter((id) => id !== peer.id)
                      : [...items, peer.id],
                  )
                }
              />
              ...
              <span>
                ...
                <small>{peer.trusted ? "Trusted" : "First transfer needs approval"}</small>
              </span>
            </label>
          ))
        )}
      ...
      <footer>
        <span className="status" title={snapshot.inbox}>
          <i />
          {status}
        </span>
